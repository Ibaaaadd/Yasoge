...
<style>
    ...
    @media(max-width:575px) {
        ...
        .inv-table tbody tr.item-row { border: 1.5px solid #fbd7aa; border-radius: 10px; padding: .75rem .75rem 2.5rem; margin-bottom: 1rem; background: #fff; position: relative; }
    ...
    @media(max-width:576px) { .inv-form-body { padding: 1rem; } }
</style>
                        ...
                        <form method="POST" action="{{ route('invoiceOut.store') }}">
                            ...
                            <script>
                                document.getElementById('tgl').addEventListener('change', function () {
                                    ...
                                    const newVal = ('YSG-OUT-' + dateStr + '-' + seq).toUpperCase();
                                                    ...
                                                    <select name="items[0][sepatu_id]"
                                                        ...
                                                        @foreach ($sepatuItems as $sepatu)
                                                            <option value="{{ $sepatu->id }}"
                                                                ...
                                                                {{ $sepatu->current_stock <= 0 ? 'disabled' : '' }}>
    ...
    <script>
        ...
        // Rebuild this row's dropdown: completely remove items used in other rows + zero-stock (unless current)
        ...
        function updateStockBadge(row) {
            ...
            if (jiInput) {
                jiInput.max = stock;
                if (parseInt(jiInput.value) > stock) jiInput.value = stock > 0 ? stock : 1;
            }
            if (badge) {
